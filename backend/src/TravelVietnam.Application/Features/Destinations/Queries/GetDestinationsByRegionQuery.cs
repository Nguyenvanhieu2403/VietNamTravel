...
{
    public class GetDestinationsByRegionQuery : IRequest<PaginatedList<DestinationDto>>
    {
    ...
    public class GetDestinationsByRegionQueryHandler : IRequestHandler<GetDestinationsByRegionQuery, PaginatedList<DestinationDto>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IMapper _mapper;
        private readonly ICacheService _cacheService;

        public GetDestinationsByRegionQueryHandler(IUnitOfWork unitOfWork, IMapper mapper, ICacheService cacheService)
        {
        ...
        public async Task<PaginatedList<DestinationDto>> Handle(GetDestinationsByRegionQuery request, CancellationToken cancellationToken)
        {
            var cacheKey = $"destinations:region:{request.RegionId}:pn_{request.PageNumber}:ps_{request.PageSize}";
            ...
            var query = _unitOfWork.Repository<Destination>().Query()
                ...
                .Where(d => !d.IsDeleted && d.RegionId == request.RegionId)
                .OrderBy(d => d.Name);
